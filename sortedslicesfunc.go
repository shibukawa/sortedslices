...
func InsertFunc[S ~[]E, E any](sorted S, item E, cmp func(e1, e2 E) int) S {
	i, _ := slices.BinarySearchFunc(sorted, item, cmp)
	if i == len(sorted)-1 && cmp(sorted[i], item) == -1 {
...
func RemoveFunc[S ~[]E, E any](sorted S, item E, cmp func(e1, e2 E) int) S {
	i, found := slices.BinarySearchFunc(sorted, item, cmp)
	if found {
...
func IterateOverFunc[S ~[]E, E any](cmp func(e1, e2 E) int, callback func(item E, srcIndex int), sorted ...S) {
	...
	for {
		...
		for i := 1; i < sourceSliceCount; i++ {
			if cmp(sourceSlices[i][indexes[i]], minItem) < 0 {
				minSlice = i
...
func UnionFunc[S ~[]E, E any](cmp func(e1, e2 E) int, sorted ...S) S {
	length := 0
	sourceSlices := make([]S, 0, len(sorted))
	for _, src := range sorted {
		if len(src) > 0 {
...
// DifferenceFunc creates difference group of sorted slices and returns.
func DifferenceFunc[S ~[]E, E any](cmp func(e1, e2 E) int, sorted1, sorted2 S) S {
	var result S
	var i, j int
	for i < len(sorted1) && j < len(sorted2) {
		compareResult := cmp(sorted1[i], sorted2[j])
		if compareResult < 0 {
...
func IntersectionFunc[S ~[]E, E any](cmp func(e1, e2 E) int, sorted ...S) S {
	...
	for _, value := range sorted[0] {
		...
		for i := 1; i < len(sorted); i++ {
			...
			for j := cursors[i]; j < len(sorted[i]); j++ {
				...
				compareResult := cmp(valueOfOtherSlice, value)
				if compareResult < 0 {
					cursors[i] = j + 1
				} else if compareResult > 0 {
					needIncrement = true
